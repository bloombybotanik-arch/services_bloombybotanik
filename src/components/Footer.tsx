...
const Footer = ({ onNavigate }: { onNavigate: (view: any, productId?: string, type?: any) => void }) => {
  const socialLinks = [
      ...
      <div className="max-w-[1200px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          <div className="col-span-1 md:col-span-2">
            <div 
              className="flex items-center gap-3 md:gap-4 mb-6 cursor-pointer group/footer-logo w-fit"
              onClick={() => onNavigate('home')}
            >
              <div className="w-14 h-14 md:w-20 md:h-20 flex items-center justify-center flex-shrink-0 overflow-hidden">
                <img src={logoSidebar} alt="Logo Bloom by BotaniK" className="w-full h-full object-contain scale-125 group-hover/footer-logo:brightness-0 group-hover/footer-logo:invert-[51%] group-hover/footer-logo:sepia-[95%] group-hover/footer-logo:saturate-[2180%] group-hover/footer-logo:hue-rotate-[1deg] group-hover/footer-logo:brightness-[101%] group-hover/footer-logo:contrast-[101%] transition-all" />
              </div>
            ...
            <p className="text-white/60 text-sm leading-relaxed max-w-md mb-8">
              L'écosystème souverain dédié à l'extraction végétale de précision. 
              Votre corps n'est pas cassé, il est verrouillé. 
              Libérez votre pharmacie intérieure avec la BloomLab.
            </p>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-white/60">
              <li><button onClick={() => onNavigate('article')} className="hover:text-white transition-colors">Le Protocole</button></li>
              <li><button onClick={() => onNavigate('boutique')} className="hover:text-white transition-colors">Boutique BloomLab</button></li>
              <li><button onClick={() => onNavigate('culinaire')} className="hover:text-white transition-colors">Atelier Culinaire</button></li>
              <li><button onClick={() => onNavigate('library')} className="hover:text-white transition-colors">Bibliothèque</button></li>
              <li><a href="https://blog.bloombybotanik.com/" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors">Le Blog</a></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-white/60">
              <li><button onClick={() => onNavigate('legal', undefined, 'mentions')} className="hover:text-white transition-colors">Mentions Légales</button></li>
              <li><button onClick={() => onNavigate('legal', undefined, 'cgv')} className="hover:text-white transition-colors">CGV</button></li>
              <li><button onClick={() => onNavigate('legal', undefined, 'cgu')} className="hover:text-white transition-colors">CGU</button></li>
              <li><button onClick={() => onNavigate('legal', undefined, 'privacy')} className="hover:text-white transition-colors">Confidentialité</button></li>
