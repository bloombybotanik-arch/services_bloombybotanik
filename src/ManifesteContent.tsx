...
import { FAMILLES_DE_STRESS } from './data/lexiqueDefinitions';
...
export default function ManifesteContent({ onBack, onNavigate, lang }: ManifesteContentProps) {
  ...
  // Schema.org FAQPage JSON-LD for SEO & Rich Snippets
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: "Qu'est-ce que la charge allostatique ?",
        acceptedAnswer: {
          '@type': 'Answer',
          text: "La charge allostatique désigne l'usure biologique accumulée par le corps lorsqu'il doit s'adapter trop souvent, trop longtemps, à trop de stress à la fois (émotionnels, hormonaux, inflammatoires, toxiques, métaboliques). Quand cette charge dépasse sa capacité de récupération, le corps ne parvient plus à retrouver son équilibre d'origine."
        }
      },
      {
        '@type': 'Question',
        name: "Quelle différence entre stress et charge allostatique ?",
        acceptedAnswer: {
          '@type': 'Answer',
          text: "Le stress est la réaction ponctuelle d'adaptation d'un organisme face à un défi immédiat. La charge allostatique est le prix cumulatif et silencieux de cette adaptation répétée dans le temps, lorsque les périodes de repos font défaut et que les systèmes de régulation finissent par s'épuiser."
        }
      },
      {
        '@type': 'Question',
        name: "Peut-on réduire sa charge allostatique ?",
        acceptedAnswer: {
          '@type': 'Answer',
          text: "Oui, en accompagnant le terrain biologique par des gestes de décharge ciblés : respecter le sommeil circadien, soutenir les voies d'élimination émonctorielles (foie, reins, intestins), apaiser les cascades inflammatoires de bas grade par des synergies végétales de haute précision et restaurer la variabilité du système nerveux autonome. Cette démarche d'hygiène de terrain ne remplace pas un avis médical."
        }
      }
    ]
  };

  const getFamilleIcon = (iconName: string) => {
    switch (iconName) {
      case 'Brain': return <Brain className="w-5 h-5 text-[#D97706]" />;
      case 'Moon': return <Moon className="w-5 h-5 text-indigo-600" />;
      case 'Flame': return <Flame className="w-5 h-5 text-amber-600" />;
      case 'ShieldAlert': return <ShieldAlert className="w-5 h-5 text-emerald-700" />;
      case 'Activity': return <Activity className="w-5 h-5 text-rose-600" />;
      case 'Zap': return <Zap className="w-5 h-5 text-teal-600" />;
      default: return <Sparkles className="w-5 h-5 text-[#D97706]" />;
    }
  };

  return (
    <GlossaryProvider pageKey="manifeste">
            ...
            {isFR ? (
              <>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-4">
                  ...
                  <p>
                    Ce que l'on nomme hâtivement un « inconfort » est souvent la réponse adaptative la plus intelligente de votre biologie pour maintenir son équilibre face à une <TooltipLexique terme="charge-allostatique">charge allostatique</TooltipLexique> devenue trop lourde. Nous ne cherchons pas à faire taire le signal. Nous cherchons à comprendre le <TooltipLexique terme="terrain">terrain</TooltipLexique> qui l'émet — ce profil unique de vulnérabilité génétique, épigénétique et allostatique propre à chaque individu.
                  </p>
                </section>

                {/* 2. SECTION CHARGE ALLOSTATIQUE */}
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-8" id="charge-allostatique">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[#FAF7F2] border border-[#E7DFD3] flex items-center justify-center text-[#D97706]">
                      <Scale className="w-5 h-5" />
                    </div>
                    <div>
                      <span className="text-xs font-black uppercase tracking-wider text-[#D97706] block">Biologie Fondamentale</span>
                      <h2 className="text-xl sm:text-2xl lg:text-3xl font-black text-[#0F261E]">
                        La charge allostatique : le poids que votre corps porte en silence
                      </h2>
                    </div>
                  </div>

                  {/* Encadré En Langage Simple */}
                  <div className="p-6 rounded-2xl bg-[#FAF7F2] border border-[#E7DFD3] space-y-3">
                    <div className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-[#D97706]">
                      <Info className="w-4 h-4" />
                      <span>En langage simple</span>
                    </div>
                  ...
                  {/* Grille des 6 familles de stress */}
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <div className="text-xs font-black uppercase tracking-wider text-[#0F261E]">
                        Les 6 familles de stress qui saturent votre adaptabilité :
                      </div>
                      <p className="text-xs sm:text-sm text-slate-600">
                        La charge allostatique n'est pas qu'une question psychologique. Elle est le résultat d'un spectre de charges allostatiques accumulées sur plusieurs plans physiologiques :
                      </p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {FAMILLES_DE_STRESS.map((f) => (
                        <div key={f.id} className="p-4 sm:p-5 rounded-2xl bg-[#FAF7F2] border border-[#E7DFD3] space-y-2">
                          <div className="flex items-center gap-2.5 font-bold text-[#0F261E] text-sm sm:text-base">
                            {getFamilleIcon(f.icone)}
                            <span>{f.titre}</span>
                          </div>
                  ...
                  {/* Tableau Comparatif 3 colonnes : Homéostasie / Allostasie / Charge allostatique */}
                  <div className="space-y-3">
                    <div className="text-xs font-black uppercase tracking-wider text-[#0F261E]">
                      Comprendre la dynamique du vivant :
                    </div>
                      ...
                      <table className="w-full text-left border-collapse text-xs sm:text-sm">
                        <thead>
                          <tr className="bg-[#FAF7F2] border-b border-[#E7DFD3] text-[#0F261E]">
                            <th className="py-3.5 px-4 font-black uppercase tracking-wider w-1/4">Concept</th>
                            <th className="py-3.5 px-4 font-black uppercase tracking-wider w-1/2">Définition canonique</th>
                            <th className="py-3.5 px-4 font-black uppercase tracking-wider w-1/4">Analogie concrète</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-[#E7DFD3]">
                          <tr className="hover:bg-[#FAF7F2]/50">
                            <td className="py-4 px-4 font-bold text-[#1C3F34]">
                              <TooltipLexique terme="homeostasie">Homéostasie</TooltipLexique>
                            </td>
                            <td className="py-4 px-4 text-slate-700 leading-relaxed">
                              Capacité du corps à revenir à son point d'équilibre d'origine après une perturbation.
                            </td>
                            <td className="py-4 px-4 text-slate-600 italic">
                              Le pilotage gyroscopique rectifiant son cap.
                            </td>
                          </tr>
                          <tr className="hover:bg-[#FAF7F2]/50">
                            <td className="py-4 px-4 font-bold text-[#D97706]">
                              <TooltipLexique terme="allostasie">Allostasie</TooltipLexique>
                            </td>
                            <td className="py-4 px-4 text-slate-700 leading-relaxed">
                              Capacité du corps à s'adapter en déplaçant temporairement son point d'équilibre.
                            </td>
                            <td className="py-4 px-4 text-slate-600 italic">
                              Le thermostat qui adapte sa consigne en tempête.
                            </td>
                          </tr>
                          <tr className="hover:bg-[#FAF7F2]/50">
                            <td className="py-4 px-4 font-bold text-rose-700">
                              <TooltipLexique terme="charge-allostatique">Charge allostatique</TooltipLexique>
                            </td>
                            <td className="py-4 px-4 text-slate-700 leading-relaxed">
                              Usure accumulée lorsque l'adaptation est continue et dépasse les réserves physiologiques.
                            </td>
                            <td className="py-4 px-4 text-slate-600 italic">
                              Le fardeau accumulé sur le dos de l'âne.
                            </td>
                          </tr>
                        </tbody>
                      </table>
                  ...
                  {/* Encadré Transparence Épistémique */}
                  <div className="p-6 rounded-2xl bg-[#0F261E] text-white space-y-3">
                    <div className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-[#D97706]">
                      <ShieldCheck className="w-4 h-4 text-[#D97706]" />
                      <span>Transparence épistémique : ce que nous savons, ce que nous continuons à affiner</span>
                    </div>
                    <p className="text-white/80 text-xs sm:text-sm leading-relaxed">
                      Le concept de charge allostatique a été formulé par Bruce McEwen et Eliot Stellar en 1993, puis consolidé par des décennies de publications neurobiologiques (McEwen 1998, Juster et al. 2010). Chez Bloom by BotaniK, nous considérons que tout paradigme scientifique est daté, sourcé et révisable. Nous ne promettons pas d'effacer la charge allostatique d'un coup de baguette magique : nous offrons les outils d'extraction et de régulation végétale les plus purs pour redonner au corps la marge de manœuvre biologique dont il a besoin pour relancer ses propres cascades d'autorégulation.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-4 pt-2">
                    <button
                      onClick={() => navigate('lexique', 'charge-allostatique')}
                      className="w-full sm:w-auto px-6 py-3.5 bg-white hover:bg-[#FAF7F2] text-[#0F261E] border border-[#E7DFD3] rounded-full font-bold text-xs sm:text-sm shadow-xs transition-all flex items-center justify-center gap-2 cursor-pointer"
                    >
                      <BookOpen className="w-4 h-4 text-[#D97706]" />
                      <span>Explorer le Lexique complet</span>
                    </button>

                    <button
                      onClick={() => navigate('chat')}
                      className="w-full sm:w-auto px-6 py-3.5 bg-[#D97706] hover:bg-[#B45309] text-white rounded-full font-bold text-xs sm:text-sm shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer"
                    >
                      <HeartPulse className="w-4 h-4" />
                      <span>Évaluer ma charge avec ALMA</span>
                      <ArrowRight className="w-4 h-4" />
                    </button>
                  </div>
                </section>

                {/* 3. Le Premier Problème */}
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      3. Le Premier Problème : Vous avez des plantes, mais les connaissez-vous vraiment ?
                    </h2>
                  </div>
                  <p>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      4. Le Deuxième Problème : Les Méthodes Traditionnelles Sous-Exploitent la Puissance de Vos Plantes
                    </h2>
                  </div>
                  <p>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="p-5 rounded-2xl bg-[#FAF7F2] border border-[#E7DFD3] space-y-2">
                        ...
                        <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                          L'eau seule est incapable de dissoudre les <TooltipLexique terme="resines">résines</TooltipLexique>, les huiles essentielles et les principes liposolubles. En faisant une simple tisane, vous ignorez jusqu'à la moitié de la matrice végétale.
                        </p>
                      </div>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      5. La Solution BloomLab : La Fin de l'Improvisation. Le Début de la Précision.
                    </h2>
                  </div>
                  <p>
                    L'héritage des sagesses anciennes — Médecine Traditionnelle Chinoise, Ayurveda, traditions chamaniques — a compris une vérité fondamentale : la plante n'est pas une molécule, c'est un écosystème. Les phytochimistes modernes l'ont redécouvert sous le nom de <strong><TooltipLexique terme="totum">Totum végétal</TooltipLexique></strong> : la synergie complexe des actifs hydrosolubles et liposolubles.
                  </p>
                  <p>
                    <strong>BloomLab</strong> a été conçu pour faire sauter les 3 verrous des méthodes traditionnelles. Notre extracteur de précision orchestre des cycles thermiques au demi-degré près et permet le <strong><TooltipLexique terme="sequencage-ab">séquençage A/B</TooltipLexique></strong> :
                  </p>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      6. Ce Que Vous Extrayez Détermine Ce Que Votre Corps Reçoit
                    </h2>
                  </div>
                  <p>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      7. Les Protocoles Systémiques Bloom : L'Ingénierie du Terrain Biologique
                    </h2>
                  </div>
                  <p>
                  ...
                  <p className="text-xs text-slate-500 italic">
                    * Rappel de sécurité : nos protocoles d'hygiène de vie et de nutrition botanique accompagnent le terrain et ne remplacent en aucun cas un avis médical.
                  </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-5 rounded-2xl bg-[#FAF7F2] border border-[#E7DFD3] space-y-1.5">
                      ...
                      <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                        Mobiliser, conjuguer et capturer les charges circulantes en accompagnant les voies d'élimination physiologiques (<TooltipLexique terme="emonctoires">émonctoires</TooltipLexique> : foie, reins, intestins, peau) pour alléger le terrain avant toute étape profonde.
                      </p>
                    </div>

                    <div className="p-5 rounded-2xl bg-[#FAF7F2] border border-[#E7DFD3] space-y-1.5">
                      <div className="text-xs font-black uppercase tracking-wider text-[#1C3F34]">Phase 1</div>
                      <div className="font-bold text-[#0F261E]">La Capture et la Neutralisation</div>
                      <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                        Utiliser la force adsorbante des argiles et <TooltipLexique terme="binder-adsorbant">binders</TooltipLexique> minéraux pour capter les endotoxines (<TooltipLexique terme="lps-endotoxines">LPS</TooltipLexique>) dans la lumière digestive et éviter les réactions de surcharge.
                      </p>
                    </div>
                    ...
                    <div className="p-5 rounded-2xl bg-[#FAF7F2] border border-[#E7DFD3] space-y-1.5">
                      ...
                      <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                        Apaiser les cascades d'<TooltipLexique terme="inflammation-de-bas-grade">inflammation de bas grade</TooltipLexique> grâce à la synergie des <TooltipLexique terme="polyphenols-flavonoides">polyphénols</TooltipLexique> et des plantes <TooltipLexique terme="adaptogenes">adaptogènes</TooltipLexique>, respectant l'intégrité de la barrière intestinale.
                      </p>
                    </div>
                    ...
                    <div className="p-5 rounded-2xl bg-[#FAF7F2] border border-[#E7DFD3] space-y-1.5">
                      ...
                      <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
                        Nourrir le <TooltipLexique terme="microbiote-dysbiose">microbiote</TooltipLexique>, soutenir la production d'énergie mitochondriale (<TooltipLexique terme="mitochondrie-atp">ATP</TooltipLexique>) et offrir au corps les cofacteurs nécessaires pour qu'il retrouve sa pleine capacité d'autorégulation.
                      </p>
                    </div>
                  </div>
                </section>

                {/* 8. Engagement d'Éthique, de Transparence et de Rigueur */}
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-6">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[#FAF7F2] border border-[#E7DFD3] flex items-center justify-center text-[#D97706]">
                  ...
                  <p>
                    Bloom by BotaniK n'est pas un cabinet médical. Nous ne délivrons pas de diagnostics et nous ne remplaçons pas la médecine conventionnelle ou d'urgence, dont nous reconnaissons l'excellence pour les crises aiguës.
                  </p>
                  <p>
                  ...
                  <div className="p-6 rounded-2xl bg-[#FAF7F2] border border-[#E7DFD3] space-y-3">
                    ...
                    <div className="space-y-2 text-xs sm:text-sm text-slate-700">
                      ...
                      <div className="flex items-start gap-2.5">
                        <CheckCircle2 className="w-4 h-4 text-[#1C3F34] shrink-0 mt-0.5" />
                        <span>Reconnaître avec humilité les limites de ce que nous savons.</span>
                      </div>
                    </div>
                  </div>
              ...
              <>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      2. Die allostatische Last: Das unsichtbare Gewicht Ihres Körpers
                    </h2>
                  </div>
                  <p>
                    Die allostatische Last ist die kumulierte biologische Abnutzung des Körpers durch anhaltende Anpassung an multiple Stressfaktoren.
                  </p>
                </section>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      3. Das erste Problem: Kennen Sie Ihre Pflanzen wirklich?
                    </h2>
                  </div>
                  <p>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      4. Die BloomLab-Lösung: Sequentielle A/B-Extraktion
                    </h2>
                  </div>
                  <p>
              ...
              <>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-4">
                  <div className="flex items-center gap-3">
                    <Scale className="w-5 h-5 text-[#D97706]" />
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      2. Allostatic Load: The Weight Your Body Carries in Silence
                    </h2>
                  </div>
                  <p>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-4">
                  <div className="flex items-center gap-3">
                    <BookOpen className="w-5 h-5 text-[#1C3F34]" />
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      3. The First Problem: You have plants, but do you truly know them?
                    </h2>
                  </div>
                  <p>
                ...
                <section className="p-8 sm:p-10 bg-white rounded-3xl border border-[#E7DFD3] shadow-xs space-y-4">
                  <div className="flex items-center gap-3">
                    <Cpu className="w-5 h-5 text-[#D97706]" />
                    <h2 className="text-xl sm:text-2xl font-black text-[#0F261E]">
                      4. The BloomLab Solution: Sequential A/B Extraction
                    </h2>
                  </div>
                  <p>
